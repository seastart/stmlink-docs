...
interface RTCClientEvent {
    /**
     * 自己加入频道成功
     * @param whiteBoard 白板链接,可能为 null(该频道未开通白板时)
     */
...
> - 链接是一个带 query 的完整 URL(形如 `https://.../board?xxx=yyy`)。
> - `whiteBoard` 为 `null` 或空字符串时,表示当前频道没有白板,不要打开页面。
> - 同一频道内所有成员通过各自的 `onJoinSucceed` 拿到的是**同一个白板链接**,从而进入同一块白板。
