...
- **SRTC 音视频 SDK** (`zh/rtc/`) - 底层音视频流媒体 SDK，支持 iOS、Swift、Android、Windows、Web、微信小程序，以及面向服务端/嵌入式的 C SDK（`zh/rtc/capi/`，内容源自 `rtc-go/capi`）和面向服务端 AI 的 Python SDK（`zh/rtc/python/`，内容源自 `rtc-python`，PyPI 包 `srtc`）
